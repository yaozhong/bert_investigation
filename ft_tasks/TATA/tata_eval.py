...
def evaluate(net, data_loader, device, tag="DEV",  verbose=True):
	...
	for x, y, category in data_loader:
		...
		#if tag == "TEST":
		#	print(torch.argmax(outputs, dim=1).to("cpu").detach().numpy())
	...
	if verbose:
		print(" |-["+tag+" EVAL]: AUC=%.4f, Precision=%.4f, Recall=%.4f, F1=%.4f, MCC=%.4f, AUPRC=%.4f, ACC=%.4f" %(auc_score, precision, recall, f1, mcc, auprc, acc))

	return mcc, auc_score, auprc

def evaluate_withPosition(net, data_loader, device, tag="DEV",  pMat=None, verbose=True):

	pred_list, gold_list, pred_list_score = [], [], []

	for x, y, category in data_loader:

		# add the position in the test
		add_pMat = np.tile(pMat[:x.shape[1], :], (x.shape[0], 1, 1))
		x = x + add_pMat
